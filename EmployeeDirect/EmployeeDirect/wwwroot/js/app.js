...
async function handleEmployeeSubmit(e) {
    ...
    try {
        ...
        const url = isEdit 
            ? `${API_BASE_URL}/Employee/${employee.employeeID}`
        ...
        if (response.ok) {
            ...
            let errorMessage = `Error ${isEdit ? 'updating' : 'adding'} employee. Status: ${response.status}`;
        ...
        formErrorMessage.textContent = error.message || `Error ${isEdit ? 'updating' : 'adding'} employee. Please try again.`;
        ...
        saveButtonText.textContent = 'Save Employee';
    }
}
